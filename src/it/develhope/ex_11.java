...
public class ex_11 {
    ...
    public static double exercise1(String text, double expectedScore) {
        ...
        // set a maxStartScore var to 20 and bring back to 20 any major score
        int maxStartScore = 20;
        if (yourScore > maxStartScore) {
            yourScore = maxStartScore;
        };


        //create an if statement to lower by 5 the score if it contains any letter a
        if (text.contains("a") ) {
            yourScore -= 5;
        }

        //create an if statement to add 10 to score if all lowercase
        if(text.equals(text.toLowerCase())) {
            yourScore += 10;
        }


        //if the score is over 20, times it by 2
        if(yourScore > 20) {
        yourScore *=2;
        }
        // End it here
    ...
    public static double exercise2(String mealType, double weight, double expectedScore) {
        ...
        switch (mealType) {
            case "sandwitch" -> yourMealScore = 5 * weight;
            case "soup" -> yourMealScore = 3 * weight;
        }
        if(yourMealScore < 5){
          return  yourMealScore = 5;
        }
        // End it here
